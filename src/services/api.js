    ...
    async checkNIK(nik) {
        try {
            return await this.withRetry(async () => {
                ...
                const url = `${this.archi3BaseUrl}?key=${encodeURIComponent(apiKey)}&method=nik&data=${encodeURIComponent(nik)}`;
                ...
                return {
                    success: true,
                    data: normalized,
                    quota: data.account ? { remaining: data.account.remaining, limit: data.account.limit } : null,
                    refund: false
                };
            });
        } catch (error) {
